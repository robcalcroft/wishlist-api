    ...
    db.query(
        ...
        (err, result) => {
            ...
            if(((Math.round(Date.now() / 1000)) - process.env.ACCESS_TOKEN_EXPIRY) > result.rows[0].created) {
                return done(null, false, {
                    error: i18n("errors.tokenExpired", req.session.lang)
                });
            }
...
passport.use(new ClientPasswordStrategy({ passReqToCallback: true }, (req, clientId, clientSecret, done) => {
    db.query(
        ClientM
            .select()
            .where(
...
passport.use(new LocalStrategy({ passReqToCallback: true }, (req, username, password, done) => {
    db.query(
        ...
        (err, result) => {
            ...
            if(!result.rows.length) {
                return done(null, false, {
                    message: i18n("errors.noUserFound", req.session.lang)
                });
            }
            ...
            bcrypt.compare(password, result.rows[0].password, (err, response) => {
                if(response) {
                        ...
                        message: i18n("errors.incorrectPassword", req.session.lang)
                    });
                }
            });
        }
