        ...
        (authCodeError) => {
          ...
          const clientUserPromise = new Promise((resolve, reject) => {
            const clientUserQuery = ClientUser
              ...
              .toQuery();
            db.all(
              clientUserQuery.text.replace('"ClientUser".*', '*'),
              clientUserQuery.values,
              (clientUserError, clientUserResult) => {
                if (clientUserError) {
                  console.log('failed here', clientUserError, clientUserQuery);
                  return reject(clientUserError);
                }

                if (!clientUserResult.length) {
                  return reject(true); // eslint-disable-line prefer-promise-reject-errors
                }
