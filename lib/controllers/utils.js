...
function i18n(code, lang) {
    console.log("Fuck", lang);
    return require(`${path.resolve(".")}/lib/I18N/${lang || "en"}.json`)[code] || false;
}
...
function getLanguage() {
    return (req, res, next) => {
        ...
        let language = req.query.lang,
            // .toString() stops lang=1 causing issues by indexing
            acceptedLanguage = require(`${path.resolve(".")}/lib/I18N/languages.json`).find(languages => languages === language);
            console.log(acceptedLanguage);
        if(acceptedLanguage !== undefined) {
