...
6. Create a `~/.pgpass` file ([instructions](http://www.postgresql.org/docs/9.3/static/libpq-pgpass.html), [example](https://github.com/robcalcroft/wishlist-api/blob/develop/.pgpass-sample)) with entries for `postgres` and `wishlist`, you'll create the `wishlist` user in a few steps.
